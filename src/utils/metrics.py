...
def compute_uni_fid(generated, real, pooling='spatial_mean'):
    ...
    Pooling strategies (misalignment robustness):
        'spatial_mean'  — Average all 32×32 patch tokens → 1024-dim.
                          Robust to tissue-plane shifts. **Recommended.**
        'cls'           — CLS token only → 1024-dim (legacy).
        'spatial_quad'  — 4-quadrant mean-pool → 4096-dim.
        ...
        for i in range(0, len(images), 16):
            ...
            with torch.no_grad():
                ...
                if pooling == 'cls':
                    ...
                    H = W = int(N ** 0.5)
                    tokens_2d = tokens.reshape(B, H, W, D)
                    h2, w2 = H // 2, W // 2
                    q1 = tokens_2d[:, :h2, :w2, :].mean(dim=(1, 2))
                    q2 = tokens_2d[:, :h2, w2:, :].mean(dim=(1, 2))
                    q3 = tokens_2d[:, h2:, :w2, :].mean(dim=(1, 2))
                    q4 = tokens_2d[:, h2:, w2:, :].mean(dim=(1, 2))
                    feat = torch.cat([q1, q2, q3, q4], dim=1)
                else:
...
def compute_iod_metrics(generated, real, labels=None):
    ...
    for i in range(0, len(generated), 16):
        ...
        od_g = -torch.log10(g_255 / 255.0)
        od_r = -torch.log10(r_255 / 255.0)
        
        miod_gen_list.append(od_g.mean(dim=(1, 2, 3)).numpy())
        miod_real_list.append(od_r.mean(dim=(1, 2, 3)).numpy())
        iod_gen_list.append(od_g.sum(dim=(1, 2, 3)).numpy())
        iod_real_list.append(od_r.sum(dim=(1, 2, 3)).numpy())
        fod_gen_list.append(od_g.pow(alpha).mean(dim=(1, 2, 3)).numpy())
        fod_real_list.append(od_r.pow(alpha).mean(dim=(1, 2, 3)).numpy())
