#!/usr/bin/env python3
"""
Evaluate UNIStainNet on ACROBAT HDF5 dataset at native 1024×1024 resolution.

Same as eval_acrobat.py but loads data at 1024 and generates at 1024.
...
def extract_features_for_crop(uni_model, he_crop_01, spatial_pool_size=32):
    """Extract UNI features from an H&E crop (any size)."""
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate UNIStainNet on ACROBAT (1024)')
    ...
    if args.output_dir is None:
        ...
        args.output_dir = f'eval_output/acrobat_1024/{ckpt_name}'
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print(f"EVALUATION: UNIStainNet on ACROBAT — 1024×1024")
