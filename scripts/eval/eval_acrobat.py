#!/usr/bin/env python3
"""
Evaluate UNIStainNet on ACROBAT HDF5 dataset (512×512 crops).

Per-stain evaluation with stain label conditioning. Computes image quality,
DAB, and IOD metrics, then reports per-stain and macro-averaged results.
...
# All 11 patients in pipeline_test.h5
_ALL_PATIENTS = [0, 1, 10, 100, 101, 102, 103, 104, 105, 106, 107]
...
def extract_features_for_crop(uni_model, he_crop_01, spatial_pool_size=32):
    """Extract UNI features from a 512×512 H&E crop."""
...
def generate_for_stain(model, uni_model, dataloader, stain_label, guidance_scale=1.0, seed=42,
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Generating")):
        he, ihc, uni_sub_crops, labels, fnames = batch
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate UNIStainNet on ACROBAT')
    parser.add_argument('--checkpoint', type=str, required=True)
    parser.add_argument('--h5_path', type=str, required=True,
                        help='Path to pipeline_test.h5')
    parser.add_argument('--stains', nargs='+', default=['HER2', 'KI67', 'ER', 'PGR'],
                        help='Stains to evaluate')
    parser.add_argument('--eval_patients', nargs='+', type=int, default=_ALL_PATIENTS,
                        help='Patient IDs to evaluate on (default: all 11)')
    ...
    if args.output_dir is None:
        ...
        args.output_dir = f'eval_output/acrobat/{ckpt_name}'
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print(f"EVALUATION: UNIStainNet on ACROBAT")
    print(f"  Stains:   {args.stains}")
    print(f"  Patients: {args.eval_patients}")
    print("=" * 70)
    ...
    results = {
        ...
        'dataset': 'ACROBAT',
        'image_size': 512,
        'eval_patients': args.eval_patients,
        'stains': args.stains,
        'per_stain': {},
    }
    ...
    for stain in args.stains:
        ...
        dm = ACROBATDataModule(
            ...
            train_patients=[0],         # placeholder, not used for eval
            val_patients=args.eval_patients,
            stains=[stain],
